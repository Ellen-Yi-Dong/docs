...
* Is the asset related to a host (vulnerability, advisory, compliance report, etc.) and the service already workspace-aware? Use [Native, workspace-level list](/docs/building-with-kessel/reference/migration-pattern-reference/#pattern-2-native-workspace-level-list-workspace-aware-high-cardinality)
* Can the resource be conceptualized as an "asset" (customer manages CRUD; we can imagine it as being placed in a Workspace)? --> Use [Default workspace](/docs/building-with-kessel/reference/migration-pattern-reference/#pattern-3-default-workspace-non-workspace-aware-assets)
...
For instructions on deploying Kessel and related services in an ephemeral environment, see the internal documentation on InScope (requires VPN).
</Aside>
